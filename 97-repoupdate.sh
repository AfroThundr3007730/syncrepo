...
# Version 1.5 updated 20181002 by <AfroThundr>
...
# Check that the repository is mounted
    ...
    touch $lockfile
    $dmirror"$ropts" $ubuntuopts1 $ubunturepo
    rm -f $lockfile
    printf '%s: Done.\n\n' "$(date -u +%FT%TZ)" | $teelog
    ...
    touch $lockfile
    $dmirror"$ropts" $ubuntuopts2 $ubunturepo
    rm -f $lockfile
    printf '%s: Done.\n\n' "$(date -u +%FT%TZ)" | $teelog
    ...
    touch $lockfile
    $dmirror"$ropts" $debianopts1 $debianrepo
    rm -f $lockfile
    printf '%s: Done.\n\n' "$(date -u +%FT%TZ)" | $teelog
    ...
    touch $lockfile
    $dmirror"$ropts" $debianopts2 $debianrepo
    rm -f $lockfile
    printf '%s: Done.\n\n' "$(date -u +%FT%TZ)" | $teelog
    ...
    touch $lockfile
    $dmirror2 $debsecopts1 $debsecrepo
    rm -f $lockfile
    printf '%s: Done.\n\n' "$(date -u +%FT%TZ)" | $teelog
    ...
    touch $lockfile
    $dmirror2 $debsecopts2 $debsecrepo
    rm -f $lockfile
    printf '%s: Done.\n\n' "$(date -u +%FT%TZ)" | $teelog
